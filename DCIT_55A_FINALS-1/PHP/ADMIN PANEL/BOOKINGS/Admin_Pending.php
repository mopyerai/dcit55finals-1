...
$result = $conn->query("SELECT * FROM booking_tbl WHERE status = 'Pending' ORDER BY id DESC");
...
<body>
  <div class="container">
    <nav class="sidebar">
      ...
      <a href="/DCIT_55A_FINALS-1/PHP/ADMIN PANEL/BOOKINGS/Admin_Pending.php" tabindex="0" class="active">Pending Bookings</a>
        ...
        <tbody>
          <?php while($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td>
               <button class="action-button edit-button" onclick="window.location.href='Admin_Pending_EDIT.php?id=<?= $row['Id'] ?>'">Edit</button>
               <button class="action-button delete-button" onclick="if(confirm('Are you sure?')) window.location.href='Admin_Pending_DELETE.php?id=<?= $row['Id'] ?>'">Delete</button>
